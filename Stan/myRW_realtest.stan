...
model {
  for (s in 1:nsubs){
    ...
    for (t in 1:ntrials){
      ...
      target += bernoulli_lpmf(expectPain[t,s] |  (expectMu[t,s]^beta_p[s])/((expectMu[t,s]^beta_p[s])+(1-expectMu[t,s])^(beta_p[s])));
    ...
    target += lognormal_lpdf(beta_p[s] | log(20), sd_beta_p);
  ...
  target += exponential_lpdf(sd_beta_p | 1);
}
...
generated quantities{
      ...
      real <lower=0> prior_beta_p[nsubs];
      ...
      prior_sd_beta_p = exponential_rng(1);
      ...
      for (s in 1:nsubs){
        ...
        prior_beta_p[s] = lognormal_rng(log(20), prior_sd_beta_p);
        ...
        for (t in 1:ntrials){
          ...
          prior_expectPain[t,s] = bernoulli_rng((prior_expectMu[t,s]^prior_beta_p[s])/((prior_expectMu[t,s]^prior_beta_p[s])+(1-prior_expectMu[t,s])^(prior_beta_p[s])));
        
          
          post_percept[t,s] = beta_proportion_rng(painMu[t,s], precision_percept[s]);
          

          post_expectPain[t,s] = bernoulli_rng((expectMu[t,s]^beta_p[s])/((expectMu[t,s]^beta_p[s])+(1-expectMu[t,s])^(beta_p[s])));
          
          
          log_lik[t,s] = beta_proportion_lpdf(percept[t,s] | painMu[t,s], precision_percept[s])+
                         bernoulli_lpmf(expectPain[t,s] |  (expectMu[t,s]^beta_p[s])/((expectMu[t,s]^beta_p[s])+(1-expectMu[t,s])^(beta_p[s])));
